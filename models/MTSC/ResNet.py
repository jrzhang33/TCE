...
class ResNet(Module):
    ...
    def res_layer(
        ...
        BasicBlock.append(ResBlock(nf, nf , kss=kss))
        layers.append(BasicBlock[0])
        for k in range(basic_layer, num_layer):
            layer = ResBlock(nf , nf , kss=kss)
        ...
        for i in range(1,num_layer): 
            ...
            #If the dimensions are inconsistent between the input channel and input, use shortcuts
                ...
                if layers[-1].out_channel!=BasicBlock[i].in_channel:
                        layers.append(Short(layers[-1].out_channel,BasicBlock[i].in_channel) )
                layers.append(BasicBlock[i])        

            
        self.fc = nn.Linear(layers[-1].out_channel, c_out)
        
        return nn.Sequential(*layers),self.fc
